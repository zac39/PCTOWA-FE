...
import './TutorForm.css'; // Importa il file CSS per lo stile

const TutorForm = () => {
  const location = useLocation(); // Hook per ottenere lo stato passato
  const { jsonData } = location.state || {}; // Recupera i dati passati dalla pagina precedente
  console.log('Dati ricevuti dalla pagina precedente:', jsonData); // Debug: stampa i dati ricevuti
  ...
  const handleSubmit = (e) => {
    ...
    if (Object.values(finalErrors).every((err) => err === '')) {
      ...
      const combinedData = {
        ...jsonData, // Dati provenienti dalla pagina precedente
...
//TODO: fai richiesta al server per inviare i dati nuovi poi naviga alla pagina dei turni di quella azienda
      ...
      console.log('Dati finali inviati:', combinedData); // Debug: stampa i dati finali combinati
